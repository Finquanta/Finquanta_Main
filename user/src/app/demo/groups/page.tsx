...
} from "@/lib/demo/api/groups";

/**
 * Business Groups in the demo — the same page the signed-in dashboard has,
 * against the session ledger. Groups are reporting metadata only: they never
 * change the books.
 */
export default function DemoGroupsPage() {
  ...
  const load = useCallback(() => {
    ...
    Promise.all([getGroups(), getGroupReport()])
      ...
      .catch((e) => setError(demoErrorText(e, t, t("demo", "groupsLoadError"))))
  ...
  const add = async () => {
    ...
    try {
      ...
      setError(demoErrorText(e, t, t("demo", "groupsCreateError")));
    } finally {
      setSaving(false);
    }
  };
  ...
  const saveRename = async () => {
    ...
    catch (e) { setError(demoErrorText(e, t, t("demo", "groupsRenameError"))); }
  };
  ...
  /** Move an entry into another group (or out), then refresh. */
  const moveItem = async (item: GroupItem, newGroupId: string) => {
    try {
      ...
      setError(demoErrorText(e, t, t("demo", "groupsMoveError")));
    }
  };
  ...
  const entryWord = (n: number) => (n === 1 ? t("demo", "entrySingular") : t("demo", "entryPlural"));
  ...
  return (
    <div className="p-4 sm:p-6 max-w-4xl">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className={`text-xl font-bold ${text}`}>{t("demo", "groupsTitle")}</h1>
      </div>
      <p className={`text-sm mb-6 ${sub}`}>{t("demo", "groupsIntro")}</p>

      {/* Create — name + a color. */}
      <div className={`${card} p-4 mb-6`}>
        <label className={`text-xs font-semibold ${sub}`}>{t("demo", "groupsNewLabel")}</label>
        <div className="flex flex-wrap gap-2 mt-2 items-center">
          <input
            className={`${field} flex-1 min-w-[160px]`}
            placeholder={t("demo", "groupsNamePlaceholder")}
            value={name}
          ...
          >
            <Plus className="h-4 w-4" /> {t("demo", "add")}
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <p className={`text-sm ${sub}`}>{t("demo", "loading")}</p>
      ) : rows.length === 0 ? (
        <div className={`${card} p-8 text-center`}>
          <p className={`text-sm ${sub}`}>{t("demo", "groupsEmpty")}</p>
        </div>
      ) : (
              ...
              <div key={key} className={`${card} p-4`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    {renaming ? (
                      <div className="flex items-center gap-1 flex-1">
                        ...
                        <button onClick={saveRename} className="p-1 text-emerald-500" title={t("demo", "save")}><Check className="h-4 w-4" /></button>
                        <button onClick={() => setEditingId(null)} className={`p-1 ${sub}`} title={t("demo", "cancel")}><X className="h-4 w-4" /></button>
                      </div>
                    ) : r.groupId ? (
                      <>
                        <button onClick={() => startRename(r)} className={`font-semibold truncate ${text} hover:text-blue-500 text-left`} title={t("demo", "groupsRenameTitle")}>
                          {r.name}
                        </button>
                        <span className={`text-xs ${sub} shrink-0`}>· {r.entries} {entryWord(r.entries)}</span>
                        <button onClick={() => startRename(r)} className={`p-0.5 ${sub} hover:text-blue-500 shrink-0`} title={t("demo", "groupsRenameTitle")}>
                      ...
                      <>
                        ...
                        <span className={`font-semibold truncate ${sub} italic`}>{t("demo", "groupsUnassigned")}</span>
                        <span className={`text-xs ${sub} shrink-0`}>· {r.entries} {entryWord(r.entries)}</span>
                      </>
                    )}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {r.groupId && (
                      <button onClick={() => archive(r)} className={`p-1 rounded ${sub} hover:text-amber-500`} title={t("demo", "groupsArchiveTitle")}>
                        <Archive className="h-4 w-4" />
                      </button>
                    )}
                    {r.groupId && (
                      <button onClick={() => remove(r)} className={`p-1 rounded ${sub} hover:text-red-500`} title={t("demo", "groupsDeleteTitle")}>
                        <Trash2 className="h-4 w-4" />
                      </button>
                    )}
                    <button onClick={() => toggleExpand(r.groupId)} className={`p-1 ${sub}`} title={t("demo", "groupsExpandTitle")}>
                ...
                <div className="flex items-center gap-4 mt-2 text-xs">
                  <span className="inline-flex items-center gap-1 text-emerald-500">
                    <TrendingUp className="h-3.5 w-3.5" /> {t("demo", "in")} {money(r.inflow)}
                  </span>
                  <span className="inline-flex items-center gap-1 text-red-400">
                    <TrendingDown className="h-3.5 w-3.5" /> {t("demo", "out")} {money(r.outflow)}
                  </span>
                </div>
                ...
                {open && (
                  <div className={`mt-3 pt-3 border-t ${isDark ? "border-gray-700" : "border-gray-100"}`}>
                    {itemsLoading ? (
                      <p className={`text-xs ${sub}`}>{t("demo", "loading")}</p>
                    ) : items.length === 0 ? (
                      <p className={`text-xs ${sub}`}>{t("demo", "groupsNothingInside")}</p>
                    ) : (
