/**
 * Finquanta's social profiles: Instagram, X and LinkedIn, the same three in the
 * same order as the transactional email footer. One list, so the side column,
 * the closing marquee and the footer can never disagree about where they point.
 */

type IconProps = { className?: string };

export function InstagramIcon({ className }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className} aria-hidden="true">
      <rect x="2.5" y="2.5" width="19" height="19" rx="5" />
      <circle cx="12" cy="12" r="4.2" />
      <circle cx="17.4" cy="6.6" r="0.9" fill="currentColor" stroke="none" />
    </svg>
  );
}

export function XIcon({ className }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.259 5.631 5.905-5.631Zm-1.161 17.52h1.833L7.084 4.126H5.117Z" />
    </svg>
  );
}

export function LinkedInIcon({ className }: IconProps) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className} aria-hidden="true">
      <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
      <rect x="2" y="9" width="4" height="12" />
      <circle cx="4" cy="4" r="2" />
    </svg>
  );
}

/**
 * Where the email button in the side column sends people. The same address the
 * Corporate plan enquiry uses (lib/pricing.ts), chosen by the user for now.
 */
export const CONTACT_EMAIL = "jeeordahnoh@gmail.com";

export const SOCIAL_LINKS = [
  { name: "Instagram", href: "https://www.instagram.com/finquanta", Icon: InstagramIcon },
  { name: "X", href: "https://x.com/finquanta", Icon: XIcon },
  { name: "LinkedIn", href: "https://www.linkedin.com/company/finquanta/", Icon: LinkedInIcon },
];
