...
const quickReports: QuickReport[] = [
  { title: 'Custom Reports', desc: 'Generate specific reports based on needs' },
  { title: 'Compliance', desc: 'Generate reports to meet regulations like GDPR' },
  { title: 'Visual Analytics', desc: 'View data in charts and graphs' },
  { title: 'Download Data', desc: 'Export data and reports for offline use' },
];
...
export default function ReportsPage() {
  ...
  const filtered = React.useMemo(()=>{
    ...
    if(!q) return quickReports;
    return quickReports.filter(r => (r.title + ' ' + r.desc).toLowerCase().includes(q));
  }, [query]);
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          <ul className="px-6 pb-4 space-y-5">
            {filtered.map((r)=> (
              <li key={r.title} className="max-w-md">
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={healthData} margin={{ left: 10, right: 10 }}>
                  ...
                  <Area type="monotone" dataKey="score" stroke="#22c55e" strokeWidth={2} fill="url(#healthGradient)" activeDot={{ r: 4, stroke:'#16a34a', strokeWidth:2 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
